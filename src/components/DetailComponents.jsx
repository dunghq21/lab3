...
function DetailComponents({ data, theme }) {
    const param = useParams()
    const id = param.id
    const movie = data.find(m => {
        return m.id == id
    ...
    return (
        <div className='w-full'>
            {movie ?
                <div className='w-full pt-[100px] text-center sm:text-left'>
                    <div className='md:flex w-full'>
                        <img className='block mx-auto max-w-[60%] max-h-[520px] mt-[6%] sm:mx-[40px]' src={movie.Image} alt='' />
                        <div className={theme ? 'mt-[6%] text-black' : 'mt-[6%] text-white'}>
                            <h1 className='font-bold text-4xl'>{movie.Title}</h1>
                            <h2 className='text-3xl my-6'>Year: {movie.Year}</h2>
                            <h2 className='text-3xl my-6'>Nation: {movie.Nation}</h2>
                    ...
                    <p className='w-full my-3 text-3xl text-center'>Description: {movie.des}</p>
                </div>
